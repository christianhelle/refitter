...
";
        ...
        compilation = compilation.AddSyntaxTrees(
            CSharpSyntaxTree.ParseText(SourceText.From(generatedClassText, Encoding.UTF8), options));
...
");
        ...
        var nonRefitMethods = interfaceSymbol.GetMembers()
        ...
        var derivedMethods = interfaceSymbol.AllInterfaces.SelectMany(i => i.GetMembers().OfType<IMethodSymbol>())
            .ToList();

        // Look for disposable
        var disposeMethod = derivedMethods.Find(
            m => m.ContainingType?.Equals(disposableInterfaceSymbol, SymbolEqualityComparer.Default) == true);
        ...
        var derivedNonRefitMethods = derivedMethods.Except(derivedMethods, SymbolEqualityComparer.Default)
            .Cast<IMethodSymbol>()
            .ToList();
...
");
    ...
    void ProcessRefitMethod(StringBuilder source, IMethodSymbol methodSymbol, bool isTopLevel)
    {
        ...
        var genericString = genericList.Count > 0
