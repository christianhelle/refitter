...
- `GenerateVisibileFile` - a boolean indicating whether the generated file should be generated into the file sturcture of the project (`true`) or not (`false`). The default is `false`. More details [here](#choose-the-method-of-generation)
...
This source generator has for history reasons two modes of running. The setting `GenerateVisibileFile` controls this. The default is `false`, which means that the generated code is generated in the sourcegenerator normal way, so at build time. (without being visible in your filestructure). This is the recommended way of using the source generator. The other option is to set `GenerateVisibileFile` to `true`, which means that the generated code is generated in a folder called `Generated` in the same location as the `.refitter` file. 
This way should be used if:

- you are for a long time using refit and you don't want to change anything.
- your CI server has no access to the OpenAPI specification file, but you want to use the source generator.
- you want to have full control over the code that is be used in your CI server. (For example, you don't want to use the latest changes form you OpenApi source, without a pullrequest)

If one of the above points matches your situation use `GenerateVisibileFile` with the value `true`.
...
- Your API project that creates the OpenAPI specification, will create the OpenAPI specification file at build time
- The source generator pics this file up and generates the code in the `Generated` folder
- The compiler doesn't know what happend and probably will build an empty client lib project
- Your projects that will use the generated code, will fail to build because in the dll of the client lib project, is basically nothing.

> Build your solution a second time and it works
