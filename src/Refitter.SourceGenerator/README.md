...
- `GenerateVisibilFile` - a boolean indicating whether the generated file should be generated into the file sturcture of the project (`true`) or not (`false`). The default is `false`. More details [here](#choose-the-method-of-generation)
...
This source generator has for history reasons two modes of running. The setting `GenerateVisibilFile` controls this. The default is `false`, which means that the generated code is generated in the sourcegenerator normal way, so at build time. (without being visible in your filestructure). This is the recommended way of using the source generator. The other option is to set `GenerateVisibilFile` to `true`, which means that the generated code is generated in a folder called `Generated` in the same location as the `.refitter` file. 
...
If one of the above points matches your situation use `GenerateVisibilFile` with the value `true`.
