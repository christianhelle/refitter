...
public static class BuildHelper
{
    ...
    {
        return BuildCSharpWithNet80(generatedCode);
    }

    public static bool BuildCSharpWithNet80(params string[] generatedCode)
    {
    ...
    public static bool BuildCSharpWithNet90(params string[] generatedCode)
    {
        var path = Path.Combine(Directory.GetCurrentDirectory(), Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(path);
        var projectFile = Path.Combine(path, "Project.csproj");
        File.WriteAllText(projectFile, ProjectFileContents.Net90App);
