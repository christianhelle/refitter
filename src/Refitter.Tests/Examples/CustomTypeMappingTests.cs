...
public class CustomTypeMappingTests
{
    ...
    [Test]
    public async Task Can_Generate_Code_With_Custom_Type_Mappings()
    {
        string generatedCode = await GenerateCodeWithCustomMappings();
    ...
    [Test]
    public async Task Works_Without_Custom_Mappings()
    {
        string generatedCode = await GenerateCodeWithoutCustomMappings();
        generatedCode.Should().NotBeNullOrWhiteSpace();
        // Without custom mappings, custom formats should fall back to defaults
        generatedCode.Should().Contain("string CustomDateProperty");
    }

    [Test]
    public async Task Can_Build_Generated_Code_With_Custom_Mappings()
    {
        string generatedCode = await GenerateCodeWithCustomMappings();
    ...
    private static async Task<string> GenerateCodeWithCustomMappings()
    {
    ...
    private static async Task<string> GenerateCodeWithoutCustomMappings()
    {
        var swaggerFile = await SwaggerFileHelper.CreateSwaggerFile(OpenApiSpec);
        var settings = new RefitGeneratorSettings
        {
