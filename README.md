...
> **Upgrading to v2.0.0?** Review the [Breaking Changes (v2.0.0)](docs/docfx_project/articles/breaking-changes-v2-0-0.md) guide. Source-generator projects may now need an explicit `Refit` reference, and the OpenAPI parser upgrade should be treated as a regenerate-review-test migration step rather than a fully proven behavioral no-op.
...
Generated contract properties default to `PascalCase`. To preserve valid OpenAPI field names such as `payMethod_SumBank`, use `--property-naming-policy PreserveOriginal` or set `"propertyNamingPolicy": "PreserveOriginal"` in a `.refitter` file.
...
```js
{
  ...
  "returnIAsyncEnumerable": true, // Optional. Default=true. Generate IAsyncEnumerable for streaming responses
...
- `outputFolder` - a string describing a relative path to a desired output folder. In multi-file mode, this must be a directory path. Default is `./Generated`
...
- `contractsOutputFolder` - a string describing a relative path to the folder where contract files are generated. Enabling this automatically enables generating multiple files. This must be a directory path. Default is `NULL`
- `generateMultipleFiles` - a boolean indicating whether to generate multiple files instead of a single large file. Refit interface(s) are written to `RefitInterfaces.cs`, contracts to `Contracts.cs`, and dependency injection to `DependencyInjection.cs`. When using the CLI `--output` in this mode, provide a directory path. Default is `false`
...
- `collectionFormat` - The collection format for array query parameters. Possible values: `Multi`, `Csv`, `Ssv`, `Tsv`, `Pipes`. Default is `Multi`. OpenAPI 3 parameters that declare `style`/`explode` use the matching format instead (`form` + `explode: true` → `Multi`, `form` + `explode: false` → `Csv`, `pipeDelimited` → `Pipes`, `spaceDelimited` → `Ssv`)
...
- `codeGeneratorSettings` - Setting this allows customization of the NSwag generated types and contracts
  ...
  - `generateOptionalPropertiesAsNullable` - Default is false. Set this explicitly when you want optional contract properties to become nullable
  - `generateNullableReferenceTypes` - Default is false. This does not implicitly enable `generateOptionalPropertiesAsNullable`
...
To limit MSBuild generation to specific `.refitter` files, set `RefitterIncludePatterns` to a semicolon-separated list of exact file names, exact project-relative paths, or exact full paths:
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td align="center" valign="top" width="14.28%"><a href="https://github.com/kemocca"><img src="https://avatars.githubusercontent.com/u/203678923?v=4?s=100" width="100px;" alt="kemocca"/><br /><sub><b>kemocca</b></sub></a><br /><a href="https://github.com/christianhelle/refitter/issues?q=author%3Akemocca" title="Bug reports">🐛</a></td>
    </tr>
  </tbody>
</table>
